...
import { createLampLocs } from './lamp-hair.js';
...
export async function initLampScene(container, options = {}) {
 ...
 await nextFrame();
 checkAlive();
 // Sculpted locs are separated, rounded ropes that drape over the forehead.
 const hair = createLampLocs(geo, uniforms.uExposure);
 portrait.add(hair); collect(hair);
 container.dataset.hairstyle = 'sculpted-locs';
 container.dataset.hairLocks = String(hair.userData.locs);
 onProgress(.78);
 let iconsLoaded = 0;
