...
export async function initLampScene(container, options = {}) {
 ...
 for(let i=0;i<vertices.count;i++){
  ...
  const faceWidth=.865-.105*jaw;
  const neckWidth=.83-.055*Math.exp(-Math.pow((y+.90)/.70,2));
  const width=THREE.MathUtils.lerp(neckWidth,faceWidth,THREE.MathUtils.smoothstep(y,-.8,-.25));
  vertices.setX(i,x*width);
  const faceZ=z+front*(-.10*brow-.17*nose+.10*lips+.035*cheek);
  // Smooth scan folds across the bare neck; there is no collar or clothing mesh.
  const neck=THREE.MathUtils.smoothstep(y,-1.85,-1.20)*(1-THREE.MathUtils.smoothstep(y,-.65,-.35));
  const neckSurface=.08+Math.sign(z-.08)*.64*Math.sqrt(Math.max(.04,1-Math.pow(x/1.12,2)));
  vertices.setZ(i,THREE.MathUtils.lerp(faceZ,neckSurface,neck*.72));
 }
