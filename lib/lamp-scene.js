...
export async function initLampScene(container, options = {}) {
 ...
 const geo=own(source.geometry.clone());geo.computeBoundingBox();const center=geo.boundingBox.getCenter(new THREE.Vector3()),size=geo.boundingBox.getSize(new THREE.Vector3());geo.translate(-center.x,-center.y,-center.z);geo.scale(5.6/size.y,5.6/size.y,5.6/size.y);// Shape an athletic, narrower jaw while retaining the facial landmarks.
 const vertices=geo.getAttribute('position');
 for(let i=0;i<vertices.count;i++){
  const x=vertices.getX(i),y=vertices.getY(i),z=vertices.getZ(i),ax=Math.abs(x);
  const cheek=Math.exp(-Math.pow((y-.65)/1.05,2));
  const jaw=Math.exp(-Math.pow((y+.35)/.28,2));
  const hollow=Math.exp(-Math.pow((ax-.72)/.29,2)-Math.pow((y-.35)/.42,2))*THREE.MathUtils.smoothstep(z,.25,1.15);
  vertices.setX(i,x*(y>-.75?.96-.1*cheek+.05*jaw:1)-Math.sign(x)*hollow*.045);
  vertices.setZ(i,z-hollow*.18);
 }
 ...
 await nextFrame();
 checkAlive();
 // A short afro is built from small tight coils, with length tapering toward the temples.
 const hairPositions=[],hairColors=[];
 const root=new THREE.Vector3(),rootNormal=new THREE.Vector3();
 seed=94621;
 for(let lock=0;lock<330;lock++){
  do{sampler.sample(root,rootNormal)}while(root.y<1.48+Math.max(root.z,0)*.4||rootNormal.y<-.18);
  const crown=THREE.MathUtils.smoothstep(root.y,1.5,2.65);
  const length=.065+crown*(.18+random()*.16);
  const axis=rootNormal.clone().lerp(new THREE.Vector3(0,1,0),crown*.25).normalize();
  const reference=Math.abs(axis.y)>.9?new THREE.Vector3(1,0,0):new THREE.Vector3(0,1,0);
  ...
  for(let strand=0;strand<9;strand++){
   const phase=random()*Math.PI*2,turns=1.1+random()*.7;
   const radius=(.017+random()*.026)*(.5+crown*.5);
   const scatterAngle=random()*Math.PI*2,scatterRadius=random()*.058;
   const origin=root.clone().addScaledVector(axis,.012).addScaledVector(tangent,Math.cos(scatterAngle)*scatterRadius).addScaledVector(bitangent,Math.sin(scatterAngle)*scatterRadius);
   const strandLength=length*(.7+random()*.3),brightness=.11+random()*.2;
   for(let j=0;j<32;j++){
    const u=j/31,theta=phase+u*Math.PI*2*turns,envelope=.7+.3*Math.sin(u*Math.PI);
    const p=origin.clone().addScaledVector(axis,u*strandLength).addScaledVector(tangent,Math.cos(theta)*radius*envelope).addScaledVector(bitangent,Math.sin(theta)*radius*envelope);
    hairPositions.push(p.x,p.y,p.z);
    const glint=brightness*(.7+.3*u);
    hairColors.push(glint*.67,glint*.69,glint);
   }
  }
 }
 // Short hairs fade down the temples and back, following the actual scalp.
 for(let i=0;i<26000;i++){sampler.sample(pos,normal);if(pos.y<.95||pos.y>2.05||pos.z>1.0||Math.abs(pos.x)<.68)continue;const fade=THREE.MathUtils.smoothstep(pos.y,.95,2.05);if(random()>fade*.8)continue;pos.addScaledVector(normal,.008+fade*.023);hairPositions.push(pos.x,pos.y,pos.z);const tone=(.05+random()*.1)*fade;hairColors.push(tone*.7,tone*.7,tone);}
 const hairGeometry=new THREE.BufferGeometry().setAttribute('position',new THREE.Float32BufferAttribute(hairPositions,3)).setAttribute('color',new THREE.Float32BufferAttribute(hairColors,3));
 const hair=new THREE.Points(hairGeometry,new THREE.PointsMaterial({vertexColors:true,size:.009,transparent:true,opacity:.86,depthWrite:true}));hair.renderOrder=2;portrait.add(hair);

 own(hairGeometry);own(hair.material);onProgress(.78);
 let iconsLoaded = 0;
