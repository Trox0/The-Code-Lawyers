...
export function createLampBob(exposure) {
  ...
  const point = (angle, t) => {
    ...
    const z=-1.12+u*2.48;
    const rootY=1.68+1.30*Math.sqrt(Math.max(.05,1-Math.pow((z-.05)/1.53,2)));
    const root=new THREE.Vector3(-.13,rootY,z);
    const outer=1.13+.07*Math.sin(u*Math.PI);
    const endY=.12+.045*Math.sin(u*19)+.035*Math.sin(u*43);
    const front=u>.77;
    const curve=new THREE.CatmullRomCurve3([
      root,
      new THREE.Vector3(side*.56-.05,rootY+.025,z+.02),
      new THREE.Vector3(side*outer,1.73,z+(front?-.13:.02)),
      new THREE.Vector3(side*(outer+.035),.84,z+(front?-.16:.035)),
      new THREE.Vector3(side*(outer+.075),endY,z+.06)
    ],false,'centripetal');
