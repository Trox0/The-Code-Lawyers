...
export function createLampBob(exposure) {
  ...
  const point = (angle, t) => {
    ...
    const fringe=front>.66;
    const endY=fringe ? 1.98 : 1.66;
    ...
    let z = .16+1.28*front*radius;
    if(t>.53){
      const drop=(t-.53)/.47;

      y=fringe ? 1.98-drop*(.12+.20*(side+1)/2) : 1.66-drop*(1.50+.08*Math.sin(angle*3));
      x=1.12*side*radius*(1+.025*Math.sin(drop*Math.PI)-.025*drop);
      z=.16+1.28*front*radius+.04*Math.sin(drop*Math.PI);
      if(fringe){z+=.10*drop;x+=.06*drop;}
    }
